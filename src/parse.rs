...
impl Parser<'_> {
    fn new<'a>(target: &'a Target, f: &'a File, path: &'a Path) -> Parser<'a> {
        Parser {
            ...
            lnum: 1,
            tmph: [TmpIdx::NONE; (TMASK + 1) as usize],
            plink: PhiIdx::NONE,
            cur_bi: BlkIdx::NONE,
            blink: BlkIdx::NONE,
            blkh: [BlkIdx::NONE; (BMASK + 1) as usize],
            rcls: K0,
            typ: vec![],
            insb: vec![],
    ...
    fn peek_with_val(&mut self) -> RubeResult<(Token, TokVal)> {
        ...
        // Ugh, help - need clone() for Str(vec)!
        Ok((self.thead.0, self.thead.1.clone()))
    }

    fn peek_tok(&mut self) -> RubeResult<Token> {
    ...
    fn nextnl_tok(&mut self) -> RubeResult<Token> {
        Ok(self.nextnl()?.0)
    }
...
impl Parser<'_> {
    fn parserefl(&mut self, arg: bool, curf: &mut Fn) -> RubeResult<bool> {
        ...
        while self.peek_tok()? != Token::Trparen {
            if !arg && vararg {
            ...
            if self.peek_tok()? == Token::Trparen {
                break;
            }
    ...
    fn closeblk(&mut self, curf: &mut Fn) {
        let curb: &mut Blk = &mut curf.blks[self.cur_bi];
    ...
    fn parseline(&mut self, ps: PState, curf: &mut Fn) -> RubeResult<PState> {
        ...
        match t {
            ...
            Token::Tlbl => {
                ...
                if self.cur_bi != BlkIdx::NONE && curf.blks[self.cur_bi].jmp.typ == J::Jxxx {
                    self.closeblk(curf);
                    let curb: &mut Blk = &mut curf.blks[self.cur_bi];
                    curb.jmp.typ = J::Jjmp;
                    curb.s1 = new_bi;
                ...
                {
                    let new_b: &Blk = &curf.blks[new_bi];
                ...
                if self.blink == BlkIdx::NONE {
                    // First block
                    assert!(new_bi == BlkIdx::START);
                    curf.start = new_bi;
                } else {
                    curf.blks[self.cur_bi].link = new_bi;
                }
                self.cur_bi = new_bi;
                self.plink = PhiIdx::NONE;
                self.expect(Token::Tnl)?;
                return Ok(PState::PPhi);
            }
            // Return instruction - ends block
            Token::Tret => {
                curf.blks[self.cur_bi].jmp.typ = match ret_for_cls(self.rcls) {
                    None => {
                ...
                if self.peek_tok()? == Token::Tnl {
                    curf.blks[self.cur_bi].jmp.typ = J::Jret0; // is this necessary?
                } else if self.rcls != K0 {
                    let r: Ref = self.parseref(curf)?;
                    if r == R {
                        return Err(self.err("invalid return value"));
                    }
                    curf.blks[self.cur_bi].jmp.arg = r;
                }
                goto_close = true;
            }
            ...
            Token::Tjmp | Token::Tjnz => {
                if t == Token::Tjmp {
                    curf.blks[self.cur_bi].jmp.typ = J::Jjmp;
                } else {
                    curf.blks[self.cur_bi].jmp.typ = J::Jjnz;
                    let r: Ref = self.parseref(curf)?;
                    if let R = r {
                ...
                curf.blks[self.cur_bi].s1 = self.findblk(curf, &name);
                if curf.blks[self.cur_bi].jmp.typ != J::Jjmp {
                    self.expect(Token::Tcomma)?;
                    let name = self.expect(Token::Tlbl)?.as_str();
                    curf.blks[self.cur_bi].s2 = self.findblk(curf, &name);
                }
                if curf.blks[self.cur_bi].s1 == curf.start
                    || curf.blks[self.cur_bi].s2 == curf.start
                {
            ...
            Token::Thlt => {
                curf.blks[self.cur_bi].jmp.typ = J::Jhlt;
                goto_close = true;
            }
            ...
            if op_tok == Token::Tcall {
                ...
                match op_tok {
                    Token::Tphi => {
                        ...
                        let pi =
                            curf.add_phi(Phi::new(r, arg.clone(), blk.clone(), k, PhiIdx::NONE));
                        if self.plink == PhiIdx::NONE {
                            curf.blks[self.cur_bi].phi = pi;
                        } else {
...
impl Parser<'_> {
    fn typecheck(&self, f: &mut Fn) -> RubeResult<()> {
        ...
        while bi != BlkIdx::NONE {
            ...
            let mut is_jerr: bool = false;
            let r: Ref = b.jmp.arg;
            if isret(b.jmp.typ) {
                if b.jmp.typ != J::Jret0 {
                    let k: K = cls_for_ret(b.jmp.typ).unwrap();
                    is_jerr = !usecheck(f, r, k);
                }
            }
            is_jerr = is_jerr || (b.jmp.typ == J::Jjnz && !usecheck(f, r, Kw));
            if is_jerr {
    ...
    fn parsefn(&mut self, lnk: &Lnk) -> RubeResult<Fn> {
        ...
        if self.nextnl_tok()? != Token::Tlbrace {
        ...
        } else if curf.blks[self.cur_bi].jmp.typ == J::Jxxx {
        ...
        for i in 0..TMASK + 1 {
            self.tmph[i as usize] = TmpIdx::NONE;
        }
    ...
    fn parsetyp(&mut self) -> RubeResult<()> {
        ...
        if t != Token::Ttyp || self.nextnl_tok()? != Token::Teq {
        ...
        if t == Token::Tint {
            ...
            if self.nextnl_tok()? != Token::Trbrace {
    ...
    fn parsedat(&mut self, cb: fn(&Dat, &[Typ]) -> (), lnk: &mut Lnk) -> RubeResult<()> {
        ...
        if t != Token::Tglo || self.nextnl_tok()? != Token::Teq {
        ...
        match b.jmp.typ {
            ...
            J::Jjmp => {
                if b.s1 == b.link {
                    ...
                    let _ = write!(outf, "\tjmp @{}", to_s(&f.blks[b.s1].name));
                }
            }
            _ => {
                let _ = write!(outf, "\t{} ", JTOA[b.jmp.typ as usize]);
                ...
                let _ = write!(
                    outf,
                    "@{}, @{}",
                    to_s(&f.blks[b.s1].name),
                    to_s(&f.blks[b.s2].name)
                );
            }
        }
        if !skip_writeln {
